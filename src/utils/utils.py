...
def format_metric(result_dict: Dict[str, Any]) -> str:
    ...
    if 'MRR' in result_dict:
        format_str.append('{}:{:<.4f}'.format('MRR', result_dict['MRR']))
    ...
    if 'loss' in result_dict:
        format_str.append('{}:{:<.4f}'.format('LOSS', result_dict['loss']))
        
    metrics = np.unique([k.split('@')[0] for k in result_dict.keys() if 'NDCG'  in k or 'HIT' in k])
    topks = np.unique([int(k.split('@')[1]) for k in result_dict.keys()  if 'NDCG'  in k or 'HIT' in k])
