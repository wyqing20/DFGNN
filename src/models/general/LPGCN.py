...
class LPGCN(GeneralModel):
    extra_log_args = ['emb_size', 'n_layers','w_linear','encoder_type' 'encoder']
    reader='HLGCNReader'
    @staticmethod
    ...
    def __init__(self, args, corpus):
        ...
        self.norm_pass_adj_neg = self.build_adjmat(corpus.n_users, corpus.n_items, corpus.train_dis_clicked_set,self.encoder_type,selfloop_flag=True)
...
class LGCNEncoder(nn.Module):
    def __init__(self, user_count, item_count, emb_size, norm_adj,norm_high_pass_adj, norm_pass_adj_neg,encoder_type,w_linear, n_layers=3,device=None):
        ...
        self.norm_high_pass_adj=self._convert_sp_mat_to_sp_tensor(norm_pass_adj_neg).to(device)
        self.gcn_layers= nn.ModuleList([nn.Sequential(nn.Linear(self.emb_size*1,self.emb_size*1,bias=True)) for _ in range(n_layers)])
    ...
    def forward(self, users, items):
        ...
        for k in range(len(self.layers)):
            ...
            # ego_embeddings=pos_ego_embeddings
                ego_embeddings=self.gcn_layers[k](pos_ego_embeddings)
...
class LGCNEncoder2(nn.Module):
    def __init__(self, user_count, item_count, emb_size, norm_adj,norm_high_pass_adj, norm_pass_adj_neg,encoder_type,w_linear, n_layers=3,device=None):
        super(LGCNEncoder2, self).__init__()
        self.user_count = user_count
        self.item_count = item_count
        self.encoder_type=encoder_type
        self.emb_size = emb_size
    ...
    def forward(self, users, items):
        ...
        for k in range(len(self.layers)):
            pos_ego_embeddings = self.low_aggs[k](self.sparse_norm_adj, pos_ego_embeddings)
            dis_ego_embeddings=self.high_aggs[k](self.norm_neg_pass_adj,dis_ego_embeddings)
