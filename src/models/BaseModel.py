...
class BaseModel(nn.Module):
    ...
    def __init__(self, args, corpus: BaseReader):
        ...
        self.device = torch.device('cuda:0' if torch.cuda.is_available() and True     else 'cpu')
    ...
    class Dataset(BaseDataset):
        ...
        def collate_batch(self, feed_dicts: List[dict]) -> dict:
            ...
            for k in feed_dict.keys():
                if ('history_items' in k or 'prefix' in k or 'length_prompt' in k) and feed_dict[k].dtype!=torch.int64:
...
class GeneralModel(BaseModel):
    ...
    def loss(self, out_dict: dict) -> torch.Tensor:
        ...
        # loss = -torch.log(1e-10 + torch.sigmoid(pos_pred - neg_pred)).mean()
        # return loss
...
class SequentialModel(GeneralModel):
    @staticmethod
    def parse_model_args(parser):
        parser.add_argument('--history_max', type=int, default=100,
