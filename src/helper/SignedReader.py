...
class SignedReader(object):
    ...
    def _read_data(self) -> NoReturn:
        ...
        self.all_df = pd.concat([df[['user_id', 'item_id','label']] for df in self.data_df.values()])
    ...
    def filter(self):
        for key in ['train', 'dev', 'test']:
            df=self.data_df[key]
            self.data_df['n_'+key]=df[df.label==0]
            self.data_df['p_'+key]=df[df.label==1]
    ...
    def _append_his_info(self) -> NoReturn:
        ...
        self.train_dis_clicked_set=dict()
        self.user_his=dict()
        for key in ['train', 'dev', 'test']:
            df = self.data_df['p_'+key]
            n_df=self.data_df['n_'+key]
            position = list()
            for uid, iid  in zip(df['user_id'], df['item_id']):
                ...
                self.user_his[uid].append(iid)
                
                if key == 'train':
                    
                    self.train_clicked_set[uid].add(iid)
            for uid, iid in zip(n_df['user_id'], n_df['item_id']):
               
                if key == 'train':
                    if uid not in self.train_dis_clicked_set:
                        self.train_dis_clicked_set[uid]=set()
                    self.train_dis_clicked_set[uid].add(iid)
